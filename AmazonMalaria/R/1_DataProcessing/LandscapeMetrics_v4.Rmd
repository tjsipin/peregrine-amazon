---
title: "LandscapeMetrics v4"
author: "Sofie McComb"
output: html_document
date: "January 12, 2023"
---

```{r setup, include=FALSE}
knitr::opts_chunk$set(echo = TRUE)
```

This Rmd document will perform landscape metric analyses on the PanAmazonia land use raster layers by municipality for South American countries of interest within the Amazon. Performed on the R server.
## Load packages

```{r packages, warning=FALSE, message=FALSE}
setwd('~/AmazonMalaria/')

#Load the necessary packages
library(dplyr)
library(tidyverse) #datatable manipulation
library(raster) #rasters
library(sf)#shapefiles
library(landscapemetrics)#fragstats
library(rgdal) #shapefiles
library(doParallel) #parallel processing - NOT installing properly...
library(tools) #Filename manipulation
options(scipen=999) #no scientific notation
rasterOptions(tmpdir = "~/AmazonMalaria/Data/RasterTemp", tmptime=1) #temporary raster storage
```


##Establish Final Municipality shapefiles for analysis
###Read in the Land Use Data for 2000

PanAmazonian land use layer tif files for each year between 2000-2017 were stiched together by year and projected into EPSG 3857 (WGS 84 / Pseudo-Mercator -- Spherical Mercator) in ArcGIS before processing in R. Selecting just 2000 for now to establish final municipal shapefiles. Will use that shapefile to subset all other land use year layers.

NEED TO UPDATE THESE: [done]

The land use categories for each land use layer are as follows:
0- Background (Not real class)
3- Forest Formation (Forest 1)
4- Savanna Formation (Forest 1)
5- Mangrove (Forest 1)
6- Flooded Forest (Forest 1)
11- Wetland (Non Forest Natural Formation 10)
12- Grassland (Non Forest Natural Formation 10)
13- Other non forest natural formation (Non Forest Natural Formation 10)
14- Farming (Farming 14)
22- Non-vegetatated area (Non vegetated area 22)
24- Urban Infrastructure (Non vegetated area 22)
25- Other Non Vegetated Area (Non vegetated area 22)
27- Non Observed (Non Observed 27)
29- Rocky Outcrop (Non Forest Natural Formation 10)
30- Mining (Non vegetated area 22)
32- Salt Flat (Non Forest Natural Formation 10)
33- River, Lake, and Ocean (Water 26)
34- Glacier (Water 26)

[Possible reclassify later: Forest 1, Non Forest Natural Formation 10, Farming 14, Non vegetated area 22, Water 26, and Non-observed 27]


```{r landuse, message=FALSE, warning=FALSE}

#Read in land cover layers by year
lc_2000<-raster("~/AmazonMalaria/Data/Raw/PanAmazonia/PanAmazonia_Projected_v4/PanAmazonia_2000_LULC_v4_Pro.tif")
```


### Read in the Municipality Data

Municipality shapefiles for each country were projected into EPSG 3857 and cropped to the extent of the PanAmazonian layers in ArcGIS before processing in R (so that only Amazonian municipalities will be considered). Read in the municipality or equivalent data for each country for Amazonian relevant areas. Cropped so that municipalities in or intersecting the background/no data areas are currently included. Will run through analysis to figure which muni are in background areas, so they can be omitted from analysis.

```{r municipios, message=FALSE, warning=FALSE}
# read in files with municipios within PanAmazonia region
brazil_muni_v3 <- read_sf("~/AmazonMalaria/Data/1_DataProcessing/LandscapeMetrics/Municipios/Amazonian_Municipios_Projected/Brazil_Amazonian_Municipios_Projected.shp")

colombia_muni_v3 <-read_sf("~/AmazonMalaria/Data/1_DataProcessing/LandscapeMetrics/Municipios/Amazonian_Municipios_Projected/Colombia_Amazonian_Municipios_Projected.shp") %>%  ## Same as v4
  as.data.frame() %>% 
  dplyr::select(MPIOS)

peru_muni_v3<-read_sf("~/AmazonMalaria/Data/1_DataProcessing/LandscapeMetrics/Municipios/Amazonian_Municipios_Projected/Peru_Amazonian_Municipios_Projected.shp")

brazil_muni<-read_sf("~/AmazonMalaria/Data/Raw/Municipios/Municipios_Projected_v4/municipios_2010_EPSG3857.shp") %>% 
  dplyr::filter(codigo_ibg %in% brazil_muni_v3$codigo_ibg )

colombia_muni<-read_sf("~/AmazonMalaria/Data/Raw/Municipios/Municipios_Projected_v4/MGN_MPIO_POLITICO_2018_EPSG3857.shp") %>% 
  dplyr::filter(MPIO_CCNCT %in% colombia_muni_v3$MPIOS) %>% 
  dplyr::rename(MPIOS = MPIO_CCNCT) 

peru_muni<-read_sf("~/AmazonMalaria/Data/Raw/Municipios/Municipios_Projected_v4/per_admbnda_adm3_2018_EPSG3857.shp") %>% 
  dplyr::filter(IDDIST %in% peru_muni_v3$IDDIST)
```


### Crop and Mask Land Use by Municipality for 2000 to establish final Municipality shapefiles

Shapefiles had some extra counties with only background land use classification. So creating new shapefile removing these from cropping these out while crop/mask land use by municipality for 2000.

#### Brazil
```{r final muni, message=FALSE, warning=FALSE, eval = T} 

#Brazil: Municipios to Loop Through
uniq_brazil_muni<-unique(as.numeric(brazil_muni$codigo_ibg))
#Crop and mask raster to municipios
ptm <- proc.time()#Time process
for(i in 1:length(uniq_brazil_muni)){
  muni<-uniq_brazil_muni[i] #get municipio
  muni_sf<-subset(brazil_muni, brazil_muni$codigo_ibg==muni) #subset municipios dataset to municipio
  municipio <- sf:::as_Spatial(muni_sf)#sf back to sp 
  lc_crop<-raster::crop(lc_2000, extent(municipio))#crop land use raster to municipio
  lcmuni<-raster::mask(lc_crop, municipio)#mask land use raster to municipio

  if(length(unique(lcmuni))>1){ #If multiple values, even with background, save raster
    writeRaster(lcmuni, paste0("~/AmazonMalaria/Data/1_DataProcessing/LandscapeMetrics/PanAmazonia_Municipios_v4/Brazil/lcmuni_2000/", muni, ".tif"), overwrite=TRUE)
  }else if(length(unique(lcmuni))==1){ #If one value, save if value is not 0 (background)
      if(unique(lcmuni)>0){
          writeRaster(lcmuni, paste0("~/AmazonMalaria/Data/1_DataProcessing/LandscapeMetrics/PanAmazonia_Municipios_v4/Brazil/lcmuni_2000/", muni, ".tif"), overwrite=TRUE)
      }else if(unique(lcmuni)==0){
        print(paste0(muni, " is only background"))
      }else
       NULL
  }else 
    NULL

}#End the for loop

proc.time() - ptm #Stop time (about 2.5 hrs)

#Read in names of rasters in folder and subset shapefile and save-for final muni shapefile
  brazil_final_munis<-substring(list.files("~/AmazonMalaria/Data/1_DataProcessing/LandscapeMetrics/PanAmazonia_Municipios_v4/Brazil/lcmuni_2000"), 1, 7) %>% 
    as.data.frame(stringsAsFactors=FALSE)
  colnames(brazil_final_munis)<-c("codigo_ibg")
  brazil_muni_final <- right_join(brazil_muni, brazil_final_munis, by="codigo_ibg")
#Write shapefile
  write_sf(brazil_muni_final, "~/AmazonMalaria/Data/1_DataProcessing/LandscapeMetrics/Municipios_v4/Final_Amazonian_Municipios_Projected/Final_Brazil_Amazonian_Municipios_Projected.shp", delete_layer = TRUE)

```


```{r}
extent(municipio)
extent(lc_2000)
plot(extent(municipio),
     xlim = c(min(extent(municipio)[1], extent(lc_2000)[1]),
              max(extent(municipio)[2], extent(lc_2000)[2])),
     ylim = c(min(extent(municipio)[3], extent(lc_2000)[3]),
              max(extent(municipio)[4], extent(lc_2000)[4])))
plot(extent(lc_2000), add=T)
```

###Colombia

```{r final muni Colombia, message=FALSE, warning=FALSE}
#Colombia: Municipios to Loop Through
uniq_colombia_muni<-unique((colombia_muni$MPIOS)) # NOTE: using as.numeric(colombia_muni$MPIOS) causes leading zeros to vanish, creating an error
#Crop and mask raster to municipios
ptm <- proc.time()#Time process
for(i in 1:length(uniq_colombia_muni)){
  tryCatch({
    muni<-uniq_colombia_muni[i] #get municipio
    muni_sf<-subset(colombia_muni, colombia_muni$MPIOS==muni) #subset municipios dataset to municipio
    municipio <- sf:::as_Spatial(muni_sf)#sf back to sp 
    lc_crop<-raster::crop(lc_2000, extent(municipio))#crop land use raster to municipio
    lcmuni<-raster::mask(lc_crop, municipio)#mask land use raster to municipio

  if(length(unique(lcmuni))>1){ #If multiple values, even with background, save raster
    writeRaster(lcmuni, paste0("~/AmazonMalaria/Data/1_DataProcessing/LandscapeMetrics/PanAmazonia_Municipios_v4/Colombia/lcmuni_2000/", muni, ".tif"), overwrite=TRUE)
  }else if(length(unique(lcmuni))==1){ #If one value, save if value is not 0 (background)
      if(unique(lcmuni)>0){
          writeRaster(lcmuni, paste0("~/AmazonMalaria/Data/1_DataProcessing/LandscapeMetrics/PanAmazonia_Municipios_v4/Colombia/lcmuni_2000/", muni, ".tif"), overwrite=TRUE)
      }else if(unique(lcmuni)==0){
        print(paste0(muni, " is only background"))
      }else
       NULL
  }else 
    NULL
  }, error = function(e){cat('ERROR: ', conditionMessage(e), '\n')})

}#End the for loop

proc.time() - ptm #Stop time #About 15-30 minutes

#Read in names of rasters in folder and subset shapefile and save-for final muni shapefile
  colombia_final_munis <- substring(list.files("~/AmazonMalaria/Data/1_DataProcessing/LandscapeMetrics/PanAmazonia_Municipios_v4/Colombia/lcmuni_2000"), 1, 5) %>% 
    as.data.frame(stringsAsFactors=FALSE)
colnames(colombia_final_munis)<-c("MPIOS")
colombia_muni_final <- right_join(colombia_muni, colombia_final_munis, by="MPIOS")
#Write shapefile
write_sf(colombia_muni_final, "~/AmazonMalaria/Data/1_DataProcessing/LandscapeMetrics/Municipios_v4/Final_Amazonian_Municipios_Projected/Final_Colombia_Amazonian_Municipios_Projected.shp", delete_layer = TRUE)
```


###Peru

```{r flip muni Peru, message=FALSE, warning=FALSE}

#Peru: Municipios to Loop Through
uniq_peru_muni<-unique(peru_muni$ADM3_PCODE)
#Crop and mask raster to municipios
ptm <- proc.time()#Time process
for(i in 1:length(uniq_peru_muni)){
  tryCatch({
    muni<-uniq_peru_muni[i] #get municipio
    muni_sf<-subset(peru_muni, peru_muni$ADM3_PCODE==muni) #subset municipios dataset to municipio
    municipio <- sf:::as_Spatial(muni_sf) #sf back to sp 
    lc_crop<-raster::crop(lc_2000, extent(municipio))#crop land use raster to municipio
    lcmuni<-raster::mask(lc_crop, municipio)#mask land use raster to municipio
  
    if(length(unique(lcmuni))>1){ #If multiple values, even with background, save raster
      writeRaster(lcmuni, paste0("~/AmazonMalaria/Data/1_DataProcessing/LandscapeMetrics/PanAmazonia_Municipios_v4/Peru/lcmuni_2000/", muni, ".tif"), overwrite=TRUE)
    }else if(length(unique(lcmuni))==1){ #If one value, save if value is not 0 (background)
        if(unique(lcmuni)>0){
            writeRaster(lcmuni, paste0("~/AmazonMalaria/Data/1_DataProcessing/LandscapeMetrics/PanAmazonia_Municipios_v4/Peru/lcmuni_2000/", muni, ".tif"), overwrite=TRUE)
        }else if(unique(lcmuni)==0){
          print(paste0(muni, " is only background"))
        }else
         NULL
    }else 
      NULL
  }, error = function(e){cat('ERROR: ', conditionMessage(e), '\n')})
  

}#End the for loop

proc.time() - ptm #Stop time #About 15-30 minutes

#Read in names of rasters in folder and subset shapefile and save-for final muni shapefile
  peru_final_munis<-substring(list.files("~/AmazonMalaria/Data/1_DataProcessing/LandscapeMetrics/PanAmazonia_Municipios_v4/Peru/lcmuni_2000"), 1, 8) %>% 
    as.data.frame(stringsAsFactors=FALSE)
  colnames(peru_final_munis)<-c("ADM3_PCODE")
  peru_muni_final <- right_join(peru_muni, peru_final_munis, by="ADM3_PCODE")
#Write shapefile
write_sf(peru_muni_final, "~/AmazonMalaria/Data/1_DataProcessing/LandscapeMetrics/Municipios_v4/Final_Amazonian_Municipios_Projected/Final_Peru_Amazonian_Municipios_Projected.shp", delete_layer = TRUE)

```


## Crop/Mask Land Use Layers by Municipios for each Year and Country

For each year the PanAmazonian land use layers will be cropped and masked by the municipalities of each country. They will be saved as to not overwhelm memory capacity or temporary raster storage and to ease furture calculations from the layers.

### Load Data

```{r cropmask data, message=FALSE, warning=FALSE}
#Country Shapefiles
brazil_municipios<-read_sf("~/AmazonMalaria/Data/1_DataProcessing/LandscapeMetrics/Municipios_v4/Final_Amazonian_Municipios_Projected/Final_Brazil_Amazonian_Municipios_Projected.shp")
colombia_municipios<-read_sf("~/AmazonMalaria/Data/1_DataProcessing/LandscapeMetrics/Municipios_v4/Final_Amazonian_Municipios_Projected/Final_Colombia_Amazonian_Municipios_Projected.shp")
peru_municipios<-read_sf("~/AmazonMalaria/Data/1_DataProcessing/LandscapeMetrics/Municipios_v4/Final_Amazonian_Municipios_Projected/Final_Peru_Amazonian_Municipios_Projected.shp")

#Land Use Rasters [UPDATED]
lc_2000<-raster("~/AmazonMalaria/Data/Raw/PanAmazonia/PanAmazonia_Projected_v4/PanAmazonia_2000_LULC_v4_Pro.tif")
# lc_2001<-raster("~/AmazonMalaria/Data/Raw/PanAmazonia/PanAmazonia_Projected_v4/PanAmazonia_2001_LULC_v4_Pro.tif")
# lc_2002<-raster("~/AmazonMalaria/Data/Raw/PanAmazonia/PanAmazonia_Projected_v4/PanAmazonia_2002_LULC_v4_Pro.tif")
# lc_2003<-raster("~/AmazonMalaria/Data/Raw/PanAmazonia/PanAmazonia_Projected_v4/PanAmazonia_2003_LULC_v4_Pro.tif")
# lc_2004<-raster("~/AmazonMalaria/Data/Raw/PanAmazonia/PanAmazonia_Projected_v4/PanAmazonia_2004_LULC_v4_Pro.tif")
# lc_2005<-raster("~/AmazonMalaria/Data/Raw/PanAmazonia/PanAmazonia_Projected_v4/PanAmazonia_2005_LULC_v4_Pro.tif")
# lc_2006<-raster("~/AmazonMalaria/Data/Raw/PanAmazonia/PanAmazonia_Projected_v4/PanAmazonia_2006_LULC_v4_Pro.tif")
# lc_2007<-raster("~/AmazonMalaria/Data/Raw/PanAmazonia/PanAmazonia_Projected_v4/PanAmazonia_2007_LULC_v4_Pro.tif")
# lc_2008<-raster("~/AmazonMalaria/Data/Raw/PanAmazonia/PanAmazonia_Projected_v4/PanAmazonia_2008_LULC_v4_Pro.tif")
# lc_2009<-raster("~/AmazonMalaria/Data/Raw/PanAmazonia/PanAmazonia_Projected_v4/PanAmazonia_2009_LULC_v4_Pro.tif")
# lc_2010<-raster("~/AmazonMalaria/Data/Raw/PanAmazonia/PanAmazonia_Projected_v4/PanAmazonia_2010_LULC_v4_Pro.tif")
# lc_2011<-raster("~/AmazonMalaria/Data/Raw/PanAmazonia/PanAmazonia_Projected_v4/PanAmazonia_2011_LULC_v4_Pro.tif")
# lc_2012<-raster("~/AmazonMalaria/Data/Raw/PanAmazonia/PanAmazonia_Projected_v4/PanAmazonia_2012_LULC_v4_Pro.tif")
# lc_2013<-raster("~/AmazonMalaria/Data/Raw/PanAmazonia/PanAmazonia_Projected_v4/PanAmazonia_2013_LULC_v4_Pro.tif")
# lc_2014<-raster("~/AmazonMalaria/Data/Raw/PanAmazonia/PanAmazonia_Projected_v4/PanAmazonia_2014_LULC_v4_Pro.tif")
# lc_2015<-raster("~/AmazonMalaria/Data/Raw/PanAmazonia/PanAmazonia_Projected_v4/PanAmazonia_2015_LULC_v4_Pro.tif")
# lc_2016<-raster("~/AmazonMalaria/Data/Raw/PanAmazonia/PanAmazonia_Projected_v4/PanAmazonia_2016_LULC_v4_Pro.tif")
# lc_2017<-raster("~/AmazonMalaria/Data/Raw/PanAmazonia/PanAmazonia_Projected_v4/PanAmazonia_2017_LULC_v4_Pro.tif")
# lc_2018<-raster("~/AmazonMalaria/Data/Raw/PanAmazonia/PanAmazonia_Projected_v4/PanAmazonia_2018_LULC_v4_Pro.tif")
# lc_2019<-raster("~/AmazonMalaria/Data/Raw/PanAmazonia/PanAmazonia_Projected_v4/PanAmazonia_2019_LULC_v4_Pro.tif")
# lc_2020<-raster("~/AmazonMalaria/Data/Raw/PanAmazonia/PanAmazonia_Projected_v4/PanAmazonia_2020_LULC_v4_Pro.tif")


```

###Create function to crop/mask land use rasters by municipios by year for each Country

```{r cropmask functions, message=FALSE, warning=FALSE}

#Brazil Function
CropMaskBrazil<-function(year, ras){
  year <- 2000
  ras <- lc_2000
  ptm = proc.time()
  year<-year
  lc<-ras
  munis<-unique(as.numeric(brazil_municipios$codigo_ibg))#Municipios to loop through
  
  #Temporary raster directory
  dir.create (file.path("~/AmazonMalaria/Data/1_DataProcessing/LandscapeMetrics_v4/RasterTmp"), showWarnings = T)  #creates unique filepath for temp directory
  rasterOptions(tmpdir=file.path("~/AmazonMalaria/Data/1_DataProcessing/LandscapeMetrics_v4/RasterTmp")) #sets temp directory
  
  #Create directory for lcmuni_ year
  dir.create (file.path(paste0("~/AmazonMalaria/Data/1_DataProcessing/LandscapeMetrics_v4/PanAmazonia_Municipios/Brazil/lcmuni_", year)), showWarnings = T)
  
  registerDoParallel(cores=detectCores()-4)#initiate multiple cores for processing (set to under computer/server limit)
      result<-foreach(c=1:length(munis))%dopar%{
        #Subset land use raster by municipio
        muni<-munis[c] #get municipio
        muni_sf<-subset(brazil_municipios, brazil_municipios$codigo_ibg==muni) #subset municipios dataset to municipio
        municipio <- sf:::as_Spatial(muni_sf)#sf back to sp
        lc_crop<-raster::crop(lc, extent(municipio))#crop land use raster to municipio
        lcmuni<-raster::mask(lc_crop, municipio)#mask land use raster to municipio

        #Write Raster (none should be empty or all background from earlier analysis)
        
        writeRaster(lcmuni, paste0("~/AmazonMalaria/Data/1_DataProcessing/LandscapeMetrics_v4/PanAmazonia_Municipios/Brazil/lcmuni_", year, "/", muni, ".tif"),
                    overwrite=TRUE)

      }#End Parallel processing

    unlink(file.path("~/AmazonMalaria/Data/1_DataProcessing/LandscapeMetrics_v4/RasterTmp"), recursive = TRUE) #removes entire temp directory 
  }#End Brazil Function

#Colombia Function
CropMaskColombia<-function(year, ras){
  ptm = proc.time()
  year<-year
  lc<-ras
  munis<-unique(as.numeric(colombia_municipios$MPIOS))#Municipios to loop through
  
  #Temporary raster directory
  dir.create (file.path("~/AmazonMalaria/Data/1_DataProcessing/LandscapeMetrics_v4/RasterTmp"), showWarnings = T)  #creates unique filepath for temp directory
  rasterOptions(tmpdir=file.path("~/AmazonMalaria/Data/1_DataProcessing/LandscapeMetrics_v4/RasterTmp")) #sets temp directory
  
  #Create directory for lcmuni_ year
  dir.create (file.path(paste0("~/AmazonMalaria/Data/1_DataProcessing/LandscapeMetrics_v4/PanAmazonia_Municipios/Colombia/lcmuni_", year)), showWarnings = T)
  
  registerDoParallel(cores=detectCores()-4)#initiate multiple cores for processing (set to under computer/server limit)
      result<-foreach(c=1:length(munis))%dopar%{
        #Subset land use raster by municipio
        muni<-munis[c] #get municipio
        muni_sf<-subset(colombia_municipios, colombia_municipios$MPIOS==muni) #subset municipios dataset to municipio
        municipio <- sf:::as_Spatial(muni_sf)#sf back to sp
        lc_crop<-raster::crop(lc, extent(municipio))#crop land use raster to municipio
        lcmuni<-raster::mask(lc_crop, municipio)#mask land use raster to municipio

        #Write Raster (none should be empty or all background from earlier analysis)
        
        writeRaster(lcmuni, paste0("~/AmazonMalaria/Data/1_DataProcessing/LandscapeMetrics_v4/PanAmazonia_Municipios/Colombia/lcmuni_", year, "/", muni, ".tif"),
                    overwrite=TRUE)

      }#End Parallel processing

    unlink(file.path("~/AmazonMalaria/Data/1_DataProcessing/LandscapeMetrics_v4/RasterTmp"), recursive = TRUE) #removes entire temp directory 
  }#End Colombia Function

#Peru Function
CropMaskPeru<-function(year, ras){
  year<-year
  lc<-ras
  munis<-unique(peru_municipios$ADM3_PCODE)#Municipios to loop through
  
  #Temporary raster directory
  dir.create (file.path("~/AmazonMalaria/Data/1_DataProcessing/LandscapeMetrics_v4/RasterTmp"), showWarnings = T)  #creates unique filepath for temp directory
  rasterOptions(tmpdir=file.path("~/AmazonMalaria/Data/1_DataProcessing/LandscapeMetrics_v4/RasterTmp")) #sets temp directory
  
  #Create directory for lcmuni_ year
  dir.create (file.path(paste0("~/AmazonMalaria/Data/1_DataProcessing/LandscapeMetrics_v4/PanAmazonia_Municipios/Peru/lcmuni_", year)), showWarnings = T)
  
  registerDoParallel(cores=detectCores()-4)#initiate multiple cores for processing (set to under computer/server limit)
      result<-foreach(c=1:length(munis))%dopar%{
        #Subset land use raster by municipio
        muni<-munis[c] #get municipio
        muni_sf<-subset(peru_municipios, peru_municipios$ADM3_PCODE==muni) #subset municipios dataset to municipio
        municipio <- sf:::as_Spatial(muni_sf)#sf back to sp
        lc_crop<-raster::crop(lc, extent(municipio))#crop land use raster to municipio
        lcmuni<-raster::mask(lc_crop, municipio)#mask land use raster to municipio

        #Write Raster (none should be empty or all background from earlier analysis)
        
        writeRaster(lcmuni, paste0("~/AmazonMalaria/Data/1_DataProcessing/LandscapeMetrics_v4/PanAmazonia_Municipios/Peru/lcmuni_", year, "/", muni, ".tif"),
                    overwrite=TRUE)

      }#End Parallel processing

    unlink(file.path("~/AmazonMalaria/Data/1_DataProcessing/LandscapeMetrics_v4/RasterTmp"), recursive = TRUE) #removes entire temp directory 
  }#End Peru Function

```


###Apply function to crop/mask land use rasters by municipios by year for each Country
```{r applyfunctions, message=FALSE, warning=FALSE}

#Apply country-specific functions to each land use layer by year 
  #Rerun 2000 so same function applied to all
#Brazil 2000-2017 (year, ras)
CropMaskBrazil(2000, lc_2000)
# CropMaskBrazil(2001, lc_2001)
# CropMaskBrazil(2002, lc_2002)
# CropMaskBrazil(2003, lc_2003)
# CropMaskBrazil(2004, lc_2004)
# CropMaskBrazil(2005, lc_2005)
# CropMaskBrazil(2006, lc_2006)
# CropMaskBrazil(2007, lc_2007)
# CropMaskBrazil(2008, lc_2008)
# CropMaskBrazil(2009, lc_2009)
# CropMaskBrazil(2010, lc_2010)
# CropMaskBrazil(2011, lc_2011)
# CropMaskBrazil(2012, lc_2012)
# CropMaskBrazil(2013, lc_2013)
# CropMaskBrazil(2014, lc_2014)
# CropMaskBrazil(2015, lc_2015)
# CropMaskBrazil(2016, lc_2016)
# CropMaskBrazil(2017, lc_2017)
# CropMaskBrazil(2018, lc_2018)
# CropMaskBrazil(2019, lc_2019)
# CropMaskBrazil(2020, lc_2020)


#Colombia 2000-2017 (year, ras)
CropMaskColombia(2000, lc_2000)
# CropMaskColombia(2001, lc_2001)
# CropMaskColombia(2002, lc_2002)
# CropMaskColombia(2003, lc_2003)
# CropMaskColombia(2004, lc_2004)
# CropMaskColombia(2005, lc_2005)
# CropMaskColombia(2006, lc_2006)
# CropMaskColombia(2007, lc_2007)
# CropMaskColombia(2008, lc_2008)
# CropMaskColombia(2009, lc_2009)
# CropMaskColombia(2010, lc_2010)
# CropMaskColombia(2011, lc_2011)
# CropMaskColombia(2012, lc_2012)
# CropMaskColombia(2013, lc_2013)
# CropMaskColombia(2014, lc_2014)
# CropMaskColombia(2015, lc_2015)
# CropMaskColombia(2016, lc_2016)
# CropMaskColombia(2017, lc_2017)
# CropMaskColombia(2018, lc_2018)
# CropMaskColombia(2019, lc_2019)
# CropMaskColombia(2020, lc_2020)


#Peru 2000-2017 (year, ras)
CropMaskPeru(2000, lc_2000)
# CropMaskPeru(2001, lc_2001)
# CropMaskPeru(2002, lc_2002)
# CropMaskPeru(2003, lc_2003)
# CropMaskPeru(2004, lc_2004)
# CropMaskPeru(2005, lc_2005)
# CropMaskPeru(2006, lc_2006)
# CropMaskPeru(2007, lc_2007)
# CropMaskPeru(2008, lc_2008)
# CropMaskPeru(2009, lc_2009)
# CropMaskPeru(2010, lc_2010)
# CropMaskPeru(2011, lc_2011)
# CropMaskPeru(2012, lc_2012)
# CropMaskPeru(2013, lc_2013)
# CropMaskPeru(2014, lc_2014)
# CropMaskPeru(2015, lc_2015)
# CropMaskPeru(2016, lc_2016)
# CropMaskPeru(2017, lc_2017)
# CropMaskPeru(2018, lc_2018)
# CropMaskPeru(2019, lc_2019)
# CropMaskPeru(2020, lc_2020)


```



## Landscape Metrics
The PanAmazonian land use by country municipality files will now be analyzed separately for each country. Each municipality land use file for each year will be read in and analyzed using functions from the landscapemetrics package. We will examine class metrics such as percent land cover type, adjacencies, etc. A dataframe of the results from the analyses, rbinded together, will be saved for each municipality to be later stitched together. This will allow progress of the computations to be checked on. Computations will be performed separately by country and will be for each looped by municipio.

### Create function to apply landscape metrics by country and year

```{r landscapemetrics, message=FALSE, warning=FALSE}

#Landscape Metrics function calculates landscape metrics for each municipio (all classes and reclasses),
  #and saves classmetrics and adjacency matrix dataframes for each municipio
  #Performs for each country and year input (loops through crop/mask rasters)

#Landscape Metrics Function
CalculateLandscapeMetrics<-function(country, year){
  ptm <- proc.time()
  country<-country
  year<-year
  lc_munis<-list.files(paste0("~/AmazonMalaria/Data/1_DataProcessing/LandscapeMetrics_v4/PanAmazonia_Municipios/",
                                       country, "/lcmuni_", year), full.names=TRUE)
  
  #Temporary raster directory
  dir.create (file.path("~/AmazonMalaria/Data/1_DataProcessing/LandscapeMetrics_v4/RasterTmp"), showWarnings = FALSE)#unique filepath for temps
  rasterOptions(tmpdir=file.path("~/AmazonMalaria/Data/1_DataProcessing/LandscapeMetrics_v4/RasterTmp")) #sets temp directory
  
  library(raster)
  library(landscapemetrics)
  
  registerDoParallel(cores=detectCores()-4)#initiate multiple cores for processing (set to under computer/server limit)
  result<-foreach(c=1:length(lc_munis))%dopar%{ 
    
    print(paste0(c, '/', length(lc_munis)))
    
    #Subset and Read in municipio land use raster
    muni<-tools::file_path_sans_ext(basename(lc_munis[c]))
    lcmuni<-raster::raster(lc_munis[c])


    #Fragstat class and landscape metrics
      #Directions/neighborhood=4 is rook's case
    ta<-landscapemetrics::lsm_l_ta(lcmuni, directions=4)#total area of landscape in hectares
    pland<-landscapemetrics::lsm_c_pland(lcmuni, directions = 4)#percentage of landscape
    mna<-landscapemetrics::lsm_c_area_mn(lcmuni, directions = 4)#mean patch area/size in hectares
    el<-landscapemetrics::lsm_c_te(lcmuni, count_boundary = FALSE, directions = 4)#edge length in meters
    enn<-landscapemetrics::lsm_c_enn_mn(lcmuni, directions = 4, verbose = TRUE) #Mean of euclidean nearest-neighbor distance in meters
      #ENN_MN is an ’Aggregation metric’& Simple way to describe patch isolation
      #It summarises each class as the mean of each patch belonging to class i.
      #Distance to the nearest neighbouring patch of the same class i in meters 

    adj<-landscapemetrics::get_adjacencies(lcmuni, neighbourhood = 4, what="triangle")#Calculate adjacencies
      #Full matrix, triangle so adjacencies only counted once
      #Adjacency matrix describes configuration of landscape
      #A cellwise count of edges between classes
      #Count the number of times that a cell with a certain value occurs to a cell with a certain value
    
    #Combine class metrics into dataframe
    classmetrics<-as.data.frame(rbind(ta, pland, mna, el, enn))
    
    #Adjacency matrix into dataframe
    adjdf<-as.data.frame(adj) %>% #Adjacency matrix to dataframe
      tibble::rownames_to_column(var="Land_Cover_Row") %>% #Rownames to column
      gather(key="Land_Cover_Column", value="EdgeCounts", -Land_Cover_Row) %>% #Other edge types to column
      dplyr::mutate(Land_Cover_Column=str_sub(Land_Cover_Column, start=2))%>% 
        #Remove X from Column name, generated by as.data.frame
      na.omit() #Remove rows with NA (NA from using triangle so not doublecounting adjacencies)
    
    #Create directories for class metrics and adjacency matrices
   dir.create(file.path(paste0("~/AmazonMalaria/Data/1_DataProcessing/LandscapeMetrics/LandscapeMetrics_v4/PanAmaMuni_allclasses/",
                                     country, "/lcmuni_", year, "/classmetrics")), showWarnings = T, recursive = T)
   dir.create(file.path(paste0("~/AmazonMalaria/Data/1_DataProcessing/LandscapeMetrics/LandscapeMetrics_v4/PanAmaMuni_allclasses/",
                                     country, "/lcmuni_", year, "/adjacencymatrix")), showWarnings = T, recursive = T)
    
      #Save both dataframes for each municipality
      write_csv(classmetrics, paste0("~/AmazonMalaria/Data/1_DataProcessing/LandscapeMetrics/LandscapeMetrics_v4/PanAmaMuni_allclasses/",
                                     country, "/lcmuni_", year, "/classmetrics/", muni, ".csv"))
      write_csv(adjdf, paste0("~/AmazonMalaria/Data/1_DataProcessing/LandscapeMetrics/LandscapeMetrics_v4/PanAmaMuni_allclasses/",
                                     country, "/lcmuni_", year, "/adjacencymatrix/", muni, ".csv"))
      #These dataframes will later be joined across all municipios, years, and then countries,
        #and then the metrics will be cleaned-up and brought together
      
  }#End parallel processing
  unlink(file.path("~/AmazonMalaria/Data/1_DataProcessing/LandscapeMetrics_v4/RasterTmp"), recursive = TRUE) #removes entire temp directory
  
  print(proc.time() - ptm)
}#End Landscape Metrics Function    
```

### Apply function to calculate landscape metrics by country and year

```{r apply landscapemetrics, message=FALSE, warning=FALSE}
#Apply calculate landscape metric function by country and year
# #Brazil 2000-2017 (country, year) 
CalculateLandscapeMetrics("Brazil", 2000)
# CalculateLandscapeMetrics("Brazil", 2001)
# CalculateLandscapeMetrics("Brazil", 2002)
# CalculateLandscapeMetrics("Brazil", 2003)
# CalculateLandscapeMetrics("Brazil", 2004)
# CalculateLandscapeMetrics("Brazil", 2005)
# CalculateLandscapeMetrics("Brazil", 2006)
# CalculateLandscapeMetrics("Brazil", 2007)
# CalculateLandscapeMetrics("Brazil", 2008)
# CalculateLandscapeMetrics("Brazil", 2009)
# CalculateLandscapeMetrics("Brazil", 2010)
# CalculateLandscapeMetrics("Brazil", 2011)
# CalculateLandscapeMetrics("Brazil", 2012)
# CalculateLandscapeMetrics("Brazil", 2013)
# CalculateLandscapeMetrics("Brazil", 2014)
# CalculateLandscapeMetrics("Brazil", 2015)
# CalculateLandscapeMetrics("Brazil", 2016)
# CalculateLandscapeMetrics("Brazil", 2017)
# CalculateLandscapeMetrics("Brazil", 2018)
# CalculateLandscapeMetrics("Brazil", 2019)
# CalculateLandscapeMetrics("Brazil", 2020)

# #Colombia 2000-2017 (country, year) 
CalculateLandscapeMetrics("Colombia", 2000)
# CalculateLandscapeMetrics("Colombia", 2001)
# CalculateLandscapeMetrics("Colombia", 2002)
# CalculateLandscapeMetrics("Colombia", 2003)
# CalculateLandscapeMetrics("Colombia", 2004)
# CalculateLandscapeMetrics("Colombia", 2005)
# CalculateLandscapeMetrics("Colombia", 2006)
# CalculateLandscapeMetrics("Colombia", 2007)
# CalculateLandscapeMetrics("Colombia", 2008)
# CalculateLandscapeMetrics("Colombia", 2009)
# CalculateLandscapeMetrics("Colombia", 2010)
# CalculateLandscapeMetrics("Colombia", 2011)
# CalculateLandscapeMetrics("Colombia", 2012)
# CalculateLandscapeMetrics("Colombia", 2013)
# CalculateLandscapeMetrics("Colombia", 2014)
# CalculateLandscapeMetrics("Colombia", 2015)
# CalculateLandscapeMetrics("Colombia", 2016)
# CalculateLandscapeMetrics("Colombia", 2017)
# CalculateLandscapeMetrics("Colombia", 2018)
# CalculateLandscapeMetrics("Colombia", 2019)
# CalculateLandscapeMetrics("Colombia", 2020)

# #Peru 2000-2017 (country, year) 
CalculateLandscapeMetrics("Peru", 2000)
# CalculateLandscapeMetrics("Peru", 2001)
# CalculateLandscapeMetrics("Peru", 2002)
# CalculateLandscapeMetrics("Peru", 2003)
# CalculateLandscapeMetrics("Peru", 2004)
# CalculateLandscapeMetrics("Peru", 2005)
# CalculateLandscapeMetrics("Peru", 2006)
# CalculateLandscapeMetrics("Peru", 2007)
# CalculateLandscapeMetrics("Peru", 2008)
# CalculateLandscapeMetrics("Peru", 2009)
# CalculateLandscapeMetrics("Peru", 2010)
# CalculateLandscapeMetrics("Peru", 2011)
# CalculateLandscapeMetrics("Peru", 2012)
# CalculateLandscapeMetrics("Peru", 2013)
# CalculateLandscapeMetrics("Peru", 2014)
# CalculateLandscapeMetrics("Peru", 2015)
# CalculateLandscapeMetrics("Peru", 2016)
# CalculateLandscapeMetrics("Peru", 2017)
# CalculateLandscapeMetrics("Peru", 2018)
# CalculateLandscapeMetrics("Peru", 2019)
# CalculateLandscapeMetrics("Peru", 2020)
```

```{r}
catch_missing <- function(country, year){
  
  if(country == "Peru"){
    original_munis<-list.files(paste0("~/AmazonMalaria/Data/1_DataProcessing/LandscapeMetrics_v4/PanAmazonia_Municipios/Peru/lcmuni_", year), full.names=TRUE) %>% 
      stringr::str_sub(-12, -5)
    
    final_munis_adj <- list.files(paste0("~/AmazonMalaria/Data/1_DataProcessing/LandscapeMetrics/LandscapeMetrics_v4/PanAmaMuni_allclasses/Peru/lcmuni_", year, "/adjacencymatrix/"), full.names = FALSE) %>% 
      stringr::str_sub(0, -5)
    
    final_munis_class <- list.files(paste0("~/AmazonMalaria/Data/1_DataProcessing/LandscapeMetrics/LandscapeMetrics_v4/PanAmaMuni_allclasses/Peru/lcmuni_", year, "/classmetrics/"), full.names = FALSE) %>% 
      stringr::str_sub(0, -5)
    
    final_munis <- intersect(final_munis_adj, final_munis_class)
    
    missing_munis <- setdiff(original_munis, final_munis)
    
    lc_munis <- paste0("~/AmazonMalaria/Data/1_DataProcessing/LandscapeMetrics_v4/PanAmazonia_Municipios/Peru/lcmuni_", year, "/", missing_munis, ".tif")
    
    } else if (country == "Colombia"){
    original_munis<-list.files(paste0("~/AmazonMalaria/Data/1_DataProcessing/LandscapeMetrics_v4/PanAmazonia_Municipios/Colombia/lcmuni_", year), full.names=TRUE) %>% 
      stringr::str_sub(-9, -5)
    
    final_munis_adj <- list.files(paste0("~/AmazonMalaria/Data/1_DataProcessing/LandscapeMetrics/LandscapeMetrics_v4/PanAmaMuni_allclasses/Colombia/lcmuni_", year, "/adjacencymatrix/"), full.names = FALSE) %>% 
      stringr::str_sub(-9, -5)
    
    final_munis_class <- list.files(paste0("~/AmazonMalaria/Data/1_DataProcessing/LandscapeMetrics/LandscapeMetrics_v4/PanAmaMuni_allclasses/Colombia/lcmuni_", year, "/classmetrics/"), full.names = FALSE) %>% 
      stringr::str_sub(0, -5)
    
    final_munis <- intersect(final_munis_adj, final_munis_class)
    
    missing_munis <- setdiff(original_munis, final_munis)
    
    lc_munis <- paste0("~/AmazonMalaria/Data/1_DataProcessing/LandscapeMetrics_v4/PanAmazonia_Municipios/Colombia/lcmuni_", year, "/", missing_munis, ".tif")
    
    } else if (country == "Brazil"){
    original_munis<-list.files(paste0("~/AmazonMalaria/Data/1_DataProcessing/LandscapeMetrics_v4/PanAmazonia_Municipios/Brazil/lcmuni_", year), full.names=TRUE) %>% 
      stringr::str_sub(-11, -5)
    
    final_munis_adj <- list.files(paste0("~/AmazonMalaria/Data/1_DataProcessing/LandscapeMetrics/LandscapeMetrics_v4/PanAmaMuni_allclasses/Brazil/lcmuni_", year, "/adjacencymatrix/"), full.names = FALSE) %>% 
      stringr::str_sub(0, -5)
    
    final_munis_class <- list.files(paste0("~/AmazonMalaria/Data/1_DataProcessing/LandscapeMetrics/LandscapeMetrics_v4/PanAmaMuni_allclasses/Brazil/lcmuni_", year, "/classmetrics/"), full.names = FALSE) %>% 
      stringr::str_sub(0, -5)
    
    final_munis <- intersect(final_munis_adj, final_munis_class)
    
    missing_munis <- setdiff(original_munis, final_munis)
    
    lc_munis <- paste0("~/AmazonMalaria/Data/1_DataProcessing/LandscapeMetrics_v4/PanAmazonia_Municipios/Brazil/lcmuni_", year, "/", missing_munis, ".tif")
    }
  
  if (length(missing_munis) == 0){
    return((print("None missing")))
  }

  
  #Temporary raster directory
  dir.create (file.path("~/AmazonMalaria/Data/1_DataProcessing/LandscapeMetrics_v4/RasterTmp"), showWarnings = FALSE)#unique filepath for temps
  rasterOptions(tmpdir=file.path("~/AmazonMalaria/Data/1_DataProcessing/LandscapeMetrics_v4/RasterTmp")) #sets temp directory
  
  
  
  library(raster)
  library(landscapemetrics)
  
  registerDoParallel(cores=detectCores()-4)#initiate multiple cores for processing (set to under computer/server limit)
  result<-foreach(c=1:length(lc_munis))%dopar%{ 
    
    print(paste0(c, '/', length(lc_munis)))
    
    #Subset and Read in municipio land use raster
    muni<-tools::file_path_sans_ext(basename(lc_munis[c]))
    lcmuni<-raster::raster(lc_munis[c])


    #Fragstat class and landscape metrics
      #Directions/neighborhood=4 is rook's case
    ta<-landscapemetrics::lsm_l_ta(lcmuni, directions=4)#total area of landscape in hectares
    pland<-landscapemetrics::lsm_c_pland(lcmuni, directions = 4)#percentage of landscape
    mna<-landscapemetrics::lsm_c_area_mn(lcmuni, directions = 4)#mean patch area/size in hectares
    el<-landscapemetrics::lsm_c_te(lcmuni, count_boundary = FALSE, directions = 4)#edge length in meters
    enn<-landscapemetrics::lsm_c_enn_mn(lcmuni, directions = 4, verbose = TRUE) #Mean of euclidean nearest-neighbor distance in meters
      #ENN_MN is an ’Aggregation metric’& Simple way to describe patch isolation
      #It summarises each class as the mean of each patch belonging to class i.
      #Distance to the nearest neighbouring patch of the same class i in meters 

    adj<-landscapemetrics::get_adjacencies(lcmuni, neighbourhood = 4, what="triangle")#Calculate adjacencies
      #Full matrix, triangle so adjacencies only counted once
      #Adjacency matrix describes configuration of landscape
      #A cellwise count of edges between classes
      #Count the number of times that a cell with a certain value occurs to a cell with a certain value
    
    #Combine class metrics into dataframe
    classmetrics<-as.data.frame(rbind(ta, pland, mna, el, enn))
    
    #Adjacency matrix into dataframe
    adjdf<-as.data.frame(adj) %>% #Adjacency matrix to dataframe
      tibble::rownames_to_column(var="Land_Cover_Row") %>% #Rownames to column
      gather(key="Land_Cover_Column", value="EdgeCounts", -Land_Cover_Row) %>% #Other edge types to column
      dplyr::mutate(Land_Cover_Column=str_sub(Land_Cover_Column, start=2))%>% 
        #Remove X from Column name, generated by as.data.frame
      na.omit() #Remove rows with NA (NA from using triangle so not doublecounting adjacencies)
    
    #Create directories for class metrics and adjacency matrices
   dir.create(file.path(paste0("~/AmazonMalaria/Data/1_DataProcessing/LandscapeMetrics/LandscapeMetrics_v4/PanAmaMuni_allclasses/",
                                     country, "/lcmuni_", year, "/classmetrics")), showWarnings = T, recursive = T)
   dir.create(file.path(paste0("~/AmazonMalaria/Data/1_DataProcessing/LandscapeMetrics/LandscapeMetrics_v4/PanAmaMuni_allclasses/",
                                     country, "/lcmuni_", year, "/adjacencymatrix")), showWarnings = T, recursive = T)
    
      #Save both dataframes for each municipality
      write_csv(classmetrics, paste0("~/AmazonMalaria/Data/1_DataProcessing/LandscapeMetrics/LandscapeMetrics_v4/PanAmaMuni_allclasses/",
                                     country, "/lcmuni_", year, "/classmetrics/", muni, ".csv"))
      write_csv(adjdf, paste0("~/AmazonMalaria/Data/1_DataProcessing/LandscapeMetrics/LandscapeMetrics_v4/PanAmaMuni_allclasses/",
                                     country, "/lcmuni_", year, "/adjacencymatrix/", muni, ".csv"))
      #These dataframes will later be joined across all municipios, years, and then countries,
        #and then the metrics will be cleaned-up and brought together
      
  }#End parallel processing
  unlink(file.path("~/AmazonMalaria/Data/1_DataProcessing/LandscapeMetrics_v4/RasterTmp"), recursive = TRUE) #removes entire temp directory
  
  print(proc.time() - ptm)
}
```

```{r}
catch_missing("Peru", 2000)
catch_missing("Colombia", 2000)
catch_missing("Brazil", 2000)
```





###Combine municipio csvs per country-year


```{r combine municipios, message=FALSE, warning=FALSE}

#Combine municipality csvs for classmetrics and adjacencymatrix
  #Saving as two csv under year folders with municipality ID column added
  library(tidyverse)
  library(stringr)
  library(purrr)

  #Loop through country
  countries<-c("Colombia", "Peru", "Brazil")
  # countries <-c('Brazil') # TRIAL
  for (c in countries){
    country<-c
    #Loop through year
    # years<-2000:2017
    years <- 2000 # TRIAL
    for (y in years){
      year<-y
      #Loop through classmetrics and adjacency matrix
      metrics<-c("classmetrics", "adjacencymatrix")
      for (m in metrics){
        metric<-m
      #List of municipio files
      list_of_files <- list.files(path = paste0("~/AmazonMalaria/Data/1_DataProcessing/LandscapeMetrics/LandscapeMetrics_v4/PanAmaMuni_allclasses/", country, "/lcmuni_", year, "/", metric),
                                  pattern = ".csv",
                                  full.names = TRUE)
      #Read in files and combine, adding muni_ID column
      df <- list_of_files %>%
        purrr::set_names() %>% 
        purrr::map_dfr(~ read_csv(.x, col_types = cols(), col_names = TRUE),.id = "Muni_ID")
        
      #Ifelse by country for shortening file name
      if(country=="Colombia"){
        df<-df %>% dplyr::mutate(Muni_ID=stringr::str_sub(Muni_ID,-9,-5))
      }else if (country=="Brazil"){
        df<-df %>% dplyr::mutate(Muni_ID=stringr::str_sub(Muni_ID,-11,-5))
      }else if (country=="Peru"){
        df<-df %>% dplyr::mutate(Muni_ID=stringr::str_sub(Muni_ID,-12,-5))
      }else
        NULL
      
      #Add year and country column
      df<-df %>% dplyr::mutate(Year=year, Country=country) %>% dplyr::select(Muni_ID, Country, Year, everything())
      
      #Save csv
      write_csv(df, paste0("~/AmazonMalaria/Data/1_DataProcessing/LandscapeMetrics/LandscapeMetrics_v4/PanAmaMuni_allclasses/metric_csv/",metric, "/", country, "_", year, ".csv"))
      
      }#End metrics for loop
    }#End year for loop
  }#End country for loop
  


#Combine all classmetric and adjacency cvs across years and countries
  #Clean up both csvs and save (not combine because a lot of information)
    
    #Class metrics
      filelist_cm <- list.files(path = "~/AmazonMalaria/Data/1_DataProcessing/LandscapeMetrics/LandscapeMetrics_v4/PanAmaMuni_allclasses/metric_csv/classmetrics",
                                pattern = ".csv", full.names = TRUE)
      cm <- Reduce(rbind, lapply(filelist_cm, read_csv))#Combine all csvs
      #Join with classmetrics csv for column headers to gather by
      cm_names<-read_csv("~/AmazonMalaria/Data/1_DataProcessing/LandscapeMetrics/LandscapeMetrics_v4/PanAmaMuni_allclasses/metric_csv/classmetrics_column_names_v4.csv")
      classmetrics<-full_join(cm, cm_names, by=c("metric", "class")) %>% 
        dplyr::select(-c(layer, level, class, id, metric)) %>% #Remove unnecessary columns
        tidyr::spread(Metric_Name, value) %>% #long to wide format
        filter(!is.na(Country))
      #Remove rows with NA for Muni_ID and then make all NA values 0 
        #(land cover type is 0 for that Muni_ID if NA)
      classmetrics[is.na(classmetrics)] <- 0 #All NA to 0
      classmetrics<-classmetrics %>% filter(Muni_ID!=0) %>% #remove row with Muni=0 (was NA)
        dplyr::select(Muni_ID, Country, Year, Muni_TotalArea, everything()) %>%  #Move ID and total area columns to beginning
        arrange(Country, Muni_ID, Year)
      #Write classmetrics csv
      write_csv(classmetrics, "~/AmazonMalaria/Data/1_DataProcessing/LandscapeMetrics/LandscapeMetrics_v4/PanAmaMuni_allclasses/metric_csv/classmetrics.csv")

    #Adjacency Matrix
      filelist_adj <- list.files(path = "~/AmazonMalaria/Data/1_DataProcessing/LandscapeMetrics/LandscapeMetrics_v4/PanAmaMuni_allclasses/metric_csv/adjacencymatrix",
                                pattern = ".csv", full.names = TRUE)
      adj <- Reduce(rbind, lapply(filelist_adj, read_csv))#Combine all csvs
      adj_names<-read_csv("~/AmazonMalaria/Data/1_DataProcessing/LandscapeMetrics/LandscapeMetrics_v4/PanAmaMuni_allclasses/metric_csv/adjacencymatrix_column_names_v4.csv")
      adjacencymatrix<-full_join(adj, adj_names, by=c("Land_Cover_Row", "Land_Cover_Column")) %>% 
        dplyr::select(-c(Land_Cover_Row, Land_Cover_Column)) %>% #Remove unnecessary columns
        tidyr::spread(Adjacency_Name, EdgeCounts) %>% #long to wide format
        filter(!is.na(Country))
      #Remove rows with NA for Muni_ID and then make all NA values 0 
        #(land cover type is 0 for that Muni_ID if NA)
      adjacencymatrix[is.na(adjacencymatrix)] <- 0 #All NA to 0
      adjacencymatrix<-adjacencymatrix %>% filter(Muni_ID!=0) %>% #remove row with Muni=0 (was NA)
        arrange(Country, Muni_ID, Year)
      #Write adjacencymatrix csv
      write_csv(adjacencymatrix, "~/AmazonMalaria/Data/1_DataProcessing/LandscapeMetrics/LandscapeMetrics_v4/PanAmaMuni_allclasses/metric_csv/adjacencymatrix.csv")


```

