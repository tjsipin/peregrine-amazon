...
```{r data}
old_adj <- read.csv('~/AmazonMalaria/Data/1_DataProcessing/LandscapeMetrics/LandscapeMetrics/PanAmaMuni_allclasses/metric_csv/adjacencymatrix.csv') %>% 
  filter(Year %in% c(2000)) %>% 
  select(contains('forest'), Muni_ID, Country, Year)

new_adj <- read.csv('~/AmazonMalaria/Data/1_DataProcessing/LandscapeMetrics/LandscapeMetrics_v4/PanAmaMuni_allclasses/metric_csv/adjacencymatrix.csv') %>% 
  filter(Year %in% c(2000)) %>% 
  select(contains('forest'), Muni_ID, Country, Year)
```
...
```{r}
old_class <- read.csv('~/AmazonMalaria/Data/1_DataProcessing/LandscapeMetrics/LandscapeMetrics/PanAmaMuni_allclasses/metric_csv/classmetrics.csv') %>% 
  filter(Year %in% c(2000)) %>% 
  select(contains('forest'), Muni_ID, Country, Year)

new_class <- read.csv('~/AmazonMalaria/Data/1_DataProcessing/LandscapeMetrics/LandscapeMetrics_v4/PanAmaMuni_allclasses/metric_csv/classmetrics.csv') %>% 
  filter(Year %in% c(2000)) %>% 
  select(contains('forest'), Muni_ID, Country, Year)
```
...
old_class_v2 <- old_class %>% 
  select(Muni_ID, Country, Year, area_mn_forest, enn_mn_forest, te_forest, pland_forest)
...
```{r}
ggplot(merged_class,
       aes(x = pland_forest_old, y = pland_forest_new)) + 
  geom_point() + 
  geom_abline(slope = 1,
              color = 'red') +
...
ggplot(merged_class,
       aes(x = area_mn_forest_old, y = area_mn_forest_new)) + 
  geom_point() + 
  geom_abline(slope = 1,
              color = 'red') +
  geom_smooth(method = 'lm')
...
ggplot(merged_class,
       aes(x = te_forest_old, y = te_forest_new)) + 
  geom_point() + 
  geom_abline(slope = 1,
              color = 'red') +
  geom_smooth(method = 'lm')
...
ggplot(merged_class,
       aes(x = enn_mn_forest_old, y = enn_mn_forest_new)) + 
  geom_point() +
  geom_abline(slope = 1,
              color = 'red') +
  geom_smooth(method = 'lm')
...
Every  variable pair are statistically different. We can then conclude that with $\alpha = 0.05$, the two data sets are statistically different and may need to be redone.
