...
```{r}
# nu = 0
ppool_syn.0 <- multisynth(Pfal.Malaria ~ deforested, 
                          unit = Code, 
                          time = Year, 
                          nu = 0,
                          data = data.use)
...
```{r}
scmForestFragmentationFunc <- function(lagYear){
  ...
  aug.data <- data.use %>% 
    ...
    select(Code, Name, Country, Year, Pfal.Malaria, mined, intervention, everything()) %>% 
  ...
  saveRDS(ppool_syn.0, paste0("~/peregrine_amazon/SCM/deforestation/models/ppool_syn_forestfragmentation_", lagYear, ".rds"))
}

scmForestEdgeLossFunc <- function(lagYear){
  ...
  saveRDS(ppool_syn.0, paste0("~/peregrine_amazon/SCM/deforestation/models/ppool_syn_edgeloss_", lagYear, ".rds"))
}
```

```{r}
scmForestFragmentationFunc(1)
scmForestFragmentationFunc(2)
scmForestFragmentationFunc(3)
scmForestFragmentationFunc(4)
scmForestFragmentationFunc(5)

scmForestEdgeLossFunc(1)
scmForestEdgeLossFunc(2)
scmForestEdgeLossFunc(3)
scmForestEdgeLossFunc(4)
scmForestEdgeLossFunc(5)
```
